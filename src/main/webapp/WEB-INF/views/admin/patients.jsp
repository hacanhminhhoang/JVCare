...
<body class="flex min-h-screen bg-muted/30 font-sans text-ink">
    ...
    <main class="flex-1 overflow-auto p-6 md:p-10">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <div id="patientGrid" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <c:forEach var="patient" items="${patients}">
                        <div class="patient-card group rounded-xl border border-border bg-card p-6 transition hover:shadow-lg">
                                    ...
                                    <div class="mt-3">
                                        <c:choose>
                                            <c:when test="${patient.gender == 'MALE'}">
                                                <span class="inline-flex items-center gap-1 rounded-full bg-blue-100 text-blue-800 px-3 py-1 text-xs font-medium">Nam</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="inline-flex items-center gap-1 rounded-full bg-pink-100 text-pink-800 px-3 py-1 text-xs font-medium">Nữ</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                </c:if>
        ...
        <!-- Pagination - server-side, always shown (outside grid) -->
