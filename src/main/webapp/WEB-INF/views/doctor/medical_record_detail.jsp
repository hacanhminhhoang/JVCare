...
<body class="min-h-screen bg-background font-sans text-ink flex">
        ...
        <div class="mx-auto max-w-4xl p-6 md:p-10">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">Mã bệnh án điện tử: <span class="font-semibold text-brand">#${record.recordId}</span></p>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-card border border-border/60 rounded-3xl p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground block">Mã bệnh nhân</span>
                            <span class="text-base font-bold text-ink mt-1 block"><c:out value="${record.patientCode}"/></span>
                        </div>
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground block">Họ và tên bệnh nhân</span>
                            <span class="text-base font-bold text-brand uppercase mt-1 block"><c:out value="${record.patientName}"/></span>
                        </div>
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground block">Ngày khám</span>
                ...
                <div class="bg-card border border-border/60 rounded-3xl p-6 shadow-sm">
                    <div class="flex items-center gap-3 border-b border-border/40 pb-4 mb-4">
                        ...
                        <h2 class="font-display font-bold text-lg text-ink">Chỉ số sinh tồn</h2>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div class="bg-muted/10 rounded-2xl p-4 border border-border/20 text-center">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Huyết áp</span>
                            <span class="block text-lg font-bold text-ink mt-1"><c:out value="${record.bloodPressure}" default="—"/></span>
                        </div>
                        <div class="bg-muted/10 rounded-2xl p-4 border border-border/20 text-center">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Nhịp tim</span>
                            <span class="block text-lg font-bold text-ink mt-1">
                                <c:choose>
                                    <c:when test="${record.heartRate > 0}">${record.heartRate} <span class="text-xs font-semibold text-muted-foreground">bpm</span></c:when>
                                    <c:otherwise>—</c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                        <div class="bg-muted/10 rounded-2xl p-4 border border-border/20 text-center">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Nhiệt độ</span>
                            <span class="block text-lg font-bold text-ink mt-1">
                                <c:choose>
                                    <c:when test="${record.temperature > 0}">${record.temperature} <span class="text-xs font-semibold text-muted-foreground">°C</span></c:when>
                                    <c:otherwise>—</c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                        <div class="bg-muted/10 rounded-2xl p-4 border border-border/20 text-center">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Cân nặng</span>
                            <span class="block text-lg font-bold text-ink mt-1">
                                <c:choose>
                                    <c:when test="${record.weight > 0}">${record.weight} <span class="text-xs font-semibold text-muted-foreground">kg</span></c:when>
                                    <c:otherwise>—</c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                        <div class="bg-muted/10 rounded-2xl p-4 border border-border/20 text-center col-span-2 md:col-span-1">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Chiều cao</span>
                            <span class="block text-lg font-bold text-ink mt-1">
                                <c:choose>
                                    <c:when test="${record.height > 0}">${record.height} <span class="text-xs font-semibold text-muted-foreground">cm</span></c:when>
                                    <c:otherwise>—</c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                ...
                <div class="bg-card border border-border/60 rounded-3xl p-6 shadow-sm">
                    ...
                    <div class="space-y-6">
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Chẩn đoán lâm sàng</span>
                            <div class="mt-2 text-base font-bold text-ink bg-muted/5 border border-border/40 rounded-2xl p-4 leading-relaxed"><c:out value="${record.diagnosis}"/></div>
                        </div>

                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Phương án điều trị</span>
                            <div class="mt-2 text-sm text-ink bg-muted/5 border border-border/40 rounded-2xl p-4 leading-relaxed whitespace-pre-wrap"><c:out value="${record.treatmentPlan}"/></div>
                        </div>

                        <c:if test="${not empty record.notes}">
                            <div>
                                <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Ghi chú phụ</span>
                                <div class="mt-2 text-sm italic text-muted-foreground bg-muted/5 border border-border/40 rounded-2xl p-4 whitespace-pre-wrap"><c:out value="${record.notes}"/></div>
                            </div>
                        </c:if>
                    </div>
                </div>
                ...
                <div class="bg-card border border-border/60 rounded-3xl p-6 shadow-sm">
                    <div class="flex items-center justify-between border-b border-border/40 pb-4 mb-4">
                        <div class="flex items-center gap-3">
                            <span class="p-2 bg-brand/10 text-brand rounded-xl">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M7 12h10"/><path d="M12 7v10"/></svg>
                            </span>
                            <h2 class="font-display font-bold text-lg text-ink">Đơn thuốc của bệnh án</h2>
                        </div>
                        <button type="button" onclick="openModal()" class="inline-flex items-center gap-1.5 rounded-xl bg-brand px-3.5 py-2 text-xs font-bold text-brand-foreground hover:opacity-90 shadow-sm transition">
                    ...
                    <c:choose>
                        <c:when test="${empty prescriptions}">
                            <div class="p-8 text-center text-muted-foreground border border-dashed border-border/60 rounded-2xl flex flex-col items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground/60"><path d="m10.5 20.5 10-10a4.95 4.95 0 1 0-7-7l-10 10a4.95 4.95 0 1 0 7 7Z"/><path d="m8.5 8.5 7 7"/></svg>
                                ...
                                <table class="w-full text-left border-collapse">
                                    <thead>
                                        <tr class="border-b border-border/60 bg-muted/20">
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground text-center w-12">STT</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">Tên thuốc</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground w-28">Liều lượng</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground w-32">Tần suất</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground text-center w-24">Số ngày</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">Hướng dẫn sử dụng</th>
                                            <th class="p-3 text-xs font-bold uppercase tracking-wider text-muted-foreground text-center w-20">Xóa</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-border/40 text-sm">
                                        <c:forEach var="p" items="${prescriptions}" varStatus="status">
                                            <tr class="hover:bg-muted/5 transition-colors">
                                                <td class="p-3 align-middle text-center font-bold text-muted-foreground">
                                                    ${status.index + 1}
                                                </td>
                                                ...
                                                <td class="p-3 align-middle text-ink font-semibold">
                                                    <span class="inline-flex items-center rounded-lg bg-muted px-2.5 py-0.5 text-xs text-ink/80"><c:out value="${p.dosage}"/></span>
                                                </td>
                                                <td class="p-3 align-middle text-muted-foreground">
                                                    <c:out value="${p.frequency}"/>
                                                </td>
                                                <td class="p-3 align-middle text-center font-semibold text-ink">
                                                    ${p.durationDays} ngày
                                                </td>
                                                <td class="p-3 align-middle text-xs text-muted-foreground italic leading-normal">
                                                ...
                                                <td class="p-3 align-middle text-center">
                                                    <a href="${pageContext.request.contextPath}/doctor/prescriptions?action=delete&id=${p.prescriptionId}&recordId=${record.recordId}" 
                                                       ...
                                                       title="Xóa thuốc">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
                                                    </a>
                                                </td>
                                            </tr>
                                        </c:forEach>
    ...
    <div id="addMedicationModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300">
        <!-- Modal Card -->
        <div class="bg-card w-full max-w-lg rounded-3xl border border-border/60 shadow-lg transform scale-95 transition-all duration-300 flex flex-col overflow-hidden">
            <!-- Modal Header -->
            <div class="px-6 py-4 border-b border-border/60 flex items-center justify-between">
                ...
                <button type="button" onclick="closeModal()" class="p-1.5 rounded-lg text-muted-foreground hover:bg-muted/40 transition">
            ...
            <form method="post" action="${pageContext.request.contextPath}/doctor/prescriptions">
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-ink mb-1.5 flex items-center gap-0.5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="frequency" required placeholder="Ví dụ: 3 lần/ngày, sau ăn..." 
                                   class="w-full px-4 py-2.5 rounded-xl border border-border bg-card text-sm text-ink placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition">
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-ink mb-1.5">Hướng dẫn sử dụng chi tiết</label>
                ...
                <!-- Modal Form Footer -->
                <div class="px-6 py-4 bg-muted/10 border-t border-border/60 flex items-center justify-end gap-3">
