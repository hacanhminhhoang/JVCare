        ...
        <body class="min-h-screen bg-muted/30 font-sans text-ink">
            <header class="sticky top-0 z-10 flex h-16 items-center border-b border-border/60 bg-card px-6 shadow-sm">
                <a href="${pageContext.request.contextPath}/doctor/index"
                    class="mr-4 text-muted-foreground hover:text-ink">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="m15 18-6-6 6-6" />
                    </svg>
                </a>
                <span class="font-display text-xl font-bold text-brand">${empty patient ? 'Tạo hồ sơ bệnh nhân mới' :
                    'Chi tiết bệnh nhân'}</span>
            </header>

            <main class="mx-auto max-w-6xl p-6 md:p-10">
                <form action="${pageContext.request.contextPath}/doctor/patient-detail" method="POST"
                    enctype="multipart/form-data"
                    class="space-y-8 rounded-3xl border border-border/60 bg-card p-6 md:p-10 shadow-sm">
                    ...
                    <div class="grid gap-8 lg:grid-cols-3">
                                ...
                                <div class="grid gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        <input type="text" name="fullName" value="${patient.fullName}" required
                                            class="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand" />
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <select name="gender"
                                            class="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand">
                                            <option value="MALE" ${patient.gender=='MALE' ? 'selected' : '' }>Nam
                                            </option>
                                            <option value="FEMALE" ${patient.gender=='FEMALE' ? 'selected' : '' }>Nữ
                                            </option>
                                            <option value="OTHER" ${patient.gender=='OTHER' ? 'selected' : '' }>Khác
                                            </option>
                                        </select>
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="mb-1 block text-sm font-semibold text-ink">Quê quán / Địa
                                            chỉ</label>
                                ...
                                <div class="grid gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        <textarea name="allergies" rows="2"
                                            class="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand">${patient.allergies}</textarea>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-sm font-semibold text-ink">Bệnh nền</label>
                                        <textarea name="chronicDiseases" rows="2"
                                            class="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand">${patient.chronicDiseases}</textarea>
                                    </div>
                                </div>
                            </div>

                            <div class="border-t border-border/60 pt-8">
                                <h2 class="mb-4 font-display text-lg font-bold text-ink">Chỉ số sức khỏe (Vital Signs)
                                </h2>
                                <p class="mb-6 text-sm text-muted-foreground">Các chỉ số này sẽ được lưu vào Hồ sơ khám
                                    bệnh mới nhất. Giữ trống nếu không cập nhật.</p>
                                <div class="grid gap-6 md:grid-cols-3">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="heartRate" value="${heartRate}"
                                            class="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand" />
                                    </div>
                                </div>
                        ...
                        <div class="flex flex-col items-center border-border/60 pt-8 lg:border-l lg:pt-0 lg:pl-8">
                            <h2 class="mb-4 w-full font-display text-lg font-bold text-ink text-left">Ảnh bệnh nhân
                                (3x4)</h2>
                            <div
                                class="relative flex aspect-[3/4] w-48 flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-border bg-muted/30">
                                <c:choose>
                                    <c:when test="${not empty patient.avatarUrl}">
                                        <img id="avatarPreview" src="${patient.avatarUrl}" alt="Avatar"
                            ...
                            <label
                                class="mt-4 cursor-pointer rounded-xl bg-brand-soft px-4 py-2 text-sm font-semibold text-brand hover:bg-brand hover:text-brand-foreground transition">
                                Chọn ảnh mới
                                <input type="file" name="avatar" accept="image/*" class="hidden"
                                    onchange="previewImage(this)" />
                            </label>
                            <p class="mt-2 text-center text-xs text-muted-foreground">Khuyến nghị ảnh tỷ lệ 3x4,<br>kích
                                thước < 5MB.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 border-t border-border/60 pt-6">
                        <a href="${pageContext.request.contextPath}/doctor/index"
                            class="rounded-xl px-6 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-muted hover:text-ink">Hủy</a>
                        <button type="submit"
                            class="rounded-xl bg-brand px-6 py-2.5 text-sm font-semibold text-brand-foreground hover:opacity-90">Lưu
                            thay đổi</button>
                    </div>
                </form>
            </main>
